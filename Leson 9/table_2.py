...
seles_by_mod = defaultdict(float)
...
for row in csv_data :
    if len(row) >=5:
        ship_mode , sales = row[2] , float(row[4])
        seles_by_mod [ship_mode] += sales

print ("{:<10} {:<15}".format("Ship Mode" , "Total Sales"))
print("-" * 25)
for mode , total_sales in seles_by_mod.items():
    print("{:<10} {:<15.2f}".format(mode ,total_sales))
